...
def _dedupe_terms(values: List[str]) -> List[str]:
    """Deduplicate display strings by normalized text while preserving order."""
    out: List[str] = []
    seen = set()
    for value in values:
        raw = (value or "").strip(" .;:-")
...
def _extract_text_list_items(text_in: str, prefixes: Tuple[str, ...]) -> List[str]:
    """Extract comma-separated expectation lists from free-text notes/conditions."""
    ...
    for match in matches:
        ...
        for part in re.split(r",|;|\band\b", text_block, flags=re.IGNORECASE):
            cleaned = part.strip(" .;:-")
            if cleaned:
...
def _extract_named_table_items(value: Any, name_keys: Tuple[str, ...]) -> List[str]:
    """Extract named table items from flexible template JSON shapes."""
    if value is None:
        return []
    if isinstance(value, str):
    ...
    if isinstance(value, list):
        items: List[str] = []
        for entry in value:
            items.extend(_extract_named_table_items(entry, name_keys))
        return _dedupe_terms(items)
    if isinstance(value, dict):
        items: List[str] = []
        matched_name_key = False
        for key in name_keys:
            raw_value = value.get(key)
            if isinstance(raw_value, str):
                matched_name_key = True
                items.append(raw_value)
            elif isinstance(raw_value, list):
                matched_name_key = True
                items.extend(_extract_named_table_items(raw_value, name_keys))

        if not matched_name_key and all(isinstance(key, str) for key in value.keys()):
            scalar_values = [item for item in value.values() if not isinstance(item, (dict, list, tuple, set))]
            if scalar_values:
                items.extend(str(key) for key in value.keys())

        for nested_key in ("items", "entries", "values", "children"):
            if nested_key in value:
                items.extend(_extract_named_table_items(value.get(nested_key), name_keys))
        return _dedupe_terms(items)
    return []


def extract_table_expectations(
    node: Dict[str, Any],
    tag: str,
    notes: str,
    conds: str,
) -> Dict[str, Any]:
    """Extract expected table structure and content expectations from template metadata."""
    column_keys = (
    ...
    name_keys = ("name", "label", "row", "column", "header", "title", "value", "element")

    explicit_columns: List[str] = []
    for key in column_keys:
        explicit_columns.extend(_extract_named_table_items(node.get(key), name_keys))

    explicit_rows: List[str] = []
    for key in row_keys:
        explicit_rows.extend(_extract_named_table_items(node.get(key), name_keys))

    explicit_elements: List[str] = []
    for key in element_keys:
        explicit_elements.extend(_extract_named_table_items(node.get(key), name_keys))

    note_columns = _extract_text_list_items(
        f"{notes}\n{conds}",
        ("include", "includes", "provide", "provides", "list", "lists", "columns", "headers"),
    )
    note_rows = _extract_text_list_items(
        f"{notes}\n{conds}",
        ("rows", "row names", "expected rows", "expected row names"),
    )

    expected_columns = _dedupe_terms(explicit_columns or note_columns)
    expected_rows = _dedupe_terms(explicit_rows or note_rows)
    expected_elements = _dedupe_terms(explicit_elements)

    note_text = f"{tag} {notes} {conds}".lower()
    is_table = bool(
        expected_columns
        or expected_rows
        or expected_elements
        or "table" in note_text
        or (len(note_columns) >= 2 and any(keyword in note_text for keyword in ("include", "provide", "column", "header")))
    )
    ...
    return {
        ...
        "has_explicit_structure": bool(explicit_columns or explicit_rows or explicit_elements),
    }


def _collect_evidence_lines(evidence_snippets: List[str]) -> List[str]:
    """Split snippets into unique evidence lines for table checks."""
    lines: List[str] = []
    seen = set()
...
def _split_table_cells(line: str) -> List[str]:
    """Split a line into probable table cells."""
...
def _line_matches_term(line: str, term: str) -> bool:
    """Check whether a normalized evidence line contains a term."""
    normalized_line = _normalize_line_text(line)
    normalized_term = _normalize_line_text(term)
    return bool(normalized_line and normalized_term and normalized_term in normalized_line)


def _line_has_payload(line: str, row_name: str, expected_columns: List[str]) -> bool:
    """Check whether a matched table row/header line contains data beyond labels."""
    cells = _split_table_cells(line)
    normalized_row = _normalize_line_text(row_name)
    normalized_columns = {
        _normalize_line_text(column)
        for column in expected_columns
        if _normalize_line_text(column)
    }

    if cells:
        meaningful_cells = []
        for cell in cells:
            normalized_cell = _normalize_line_text(cell)
            if not normalized_cell:
                continue
            if normalized_row and normalized_cell == normalized_row:
                continue
            if normalized_cell in normalized_columns:
                continue
            meaningful_cells.append(normalized_cell)
        if meaningful_cells:
            return True

    residual = _normalize_line_text(line)
    if normalized_row:
        if normalized_row not in residual:
            return False
        residual = residual.replace(normalized_row, " ")
    for column in normalized_columns:
        residual = residual.replace(column, " ")

    remaining_tokens = [token for token in _tokenize_match_text(residual) if len(token) > 2]
    if normalized_row:
        return len(remaining_tokens) >= 1
    return len(remaining_tokens) >= 2


def evaluate_table_evidence(
    tag: str,
    ...
    """Deterministically validate that expected table structure is present in evidence."""
    lines = _collect_evidence_lines(evidence_snippets)
    if not lines:
        return {
            "status": "MISSING",
            "justification": f"Expected table '{tag}' was not found in the retrieved evidence.",
            "citations": [],
            "expected_columns": expected_columns,
            "expected_rows": expected_rows,
            "expected_elements": expected_elements,
    ...
    column_citations: List[str] = []
    for column in expected_columns:
        matches = [line for line in lines if _line_matches_term(line, column)]
        if matches:
            found_columns.append(column)
            column_citations.append(_truncate_text(matches[0], 200))
        else:
            missing_columns.append(column)

    header_match_threshold = max(1, min(2, len(expected_columns)))
    header_lines = [
        line
        for line in lines
        if expected_columns
        and sum(1 for column in expected_columns if _line_matches_term(line, column)) >= header_match_threshold
    ]
    normalized_header_lines = {_normalize_line_text(line) for line in header_lines}
    non_header_lines = [
        line for line in lines if _normalize_line_text(line) not in normalized_header_lines
    ]

    row_presence: List[Dict[str, Any]] = []
    complete_row_count = 0
    partial_row_count = 0
    for row_name in expected_rows:
        matches = [line for line in lines if _line_matches_term(line, row_name)]
        has_data = any(_line_has_payload(line, row_name, expected_columns) for line in matches)
        if matches:
            partial_row_count += 1
        if has_data:
        ...
        row_presence.append({
            ...
            "has_data": has_data,
            "sample": _truncate_text(matches[0], 200) if matches else "",
        })

    element_presence: List[Dict[str, Any]] = []
    present_element_count = 0
    element_candidate_lines = non_header_lines or lines
    for element_name in expected_elements:
        matches = [line for line in element_candidate_lines if _line_matches_term(line, element_name)]
        if matches:
            present_element_count += 1
        element_presence.append({
            "name": element_name,
            "present": bool(matches),
            "sample": _truncate_text(matches[0], 200) if matches else "",
        })
    ...
    structured_lines = [
        ...
        if len(_split_table_cells(line)) >= 2
    ]
    populated_non_header_lines = [
        line for line in structured_lines if _line_has_payload(line, "", expected_columns)
    ]
    data_row_count = complete_row_count if expected_rows else len(populated_non_header_lines)
    ...
    if (
        ...
        and expected_elements_present
        and (has_required_row_data or not expected_columns)
    ):
        status = "PRESENT"
    elif found_columns or partial_row_count or present_element_count or header_lines or structured_lines:
        status = "PARTIAL"
    else:
        status = "MISSING"
    ...
    if expected_columns:
        if missing_columns:
            ...
            detail_parts.append(
                f"all expected column(s) present: {', '.join(found_columns)}"
            )
    if expected_rows:
        missing_rows = [row["name"] for row in row_presence if not row["present"]]
        incomplete_rows = [row["name"] for row in row_presence if row["present"] and not row["has_data"]]
        if missing_rows:
            detail_parts.append(f"missing row(s): {', '.join(missing_rows)}")
        if incomplete_rows:
            detail_parts.append(f"row(s) without data: {', '.join(incomplete_rows)}")
    ...
    if expected_elements:
        ...
        if data_row_count:
            ...
            detail_parts.append("no populated non-header table rows found")

    justification = f"Table validation for '{tag}': {'; '.join(detail_parts) or 'no table evidence found.'}"

    citations = column_citations[:2]
    citations.extend(
        row["sample"]
        for row in row_presence
        if row.get("sample")
    )
    citations.extend(
        item["sample"]
        for item in element_presence
        if item.get("sample")
    )
    if not citations:
        citations = [_truncate_text(line, 200) for line in (header_lines or structured_lines or lines)[:2]]

    return {
        "status": status,
        "justification": justification,
        "citations": _dedupe_terms(citations)[:4],
        "expected_columns": expected_columns,
        "expected_rows": expected_rows,
        "expected_elements": expected_elements,
...
def run_validation(
    ...
    for intent_name, child in requirement_nodes:
        ...
        table_expectations = extract_table_expectations(child, tag, notes, conds)
        ...
        combined_match_terms = (
            ...
            + requirement_phrases
            + table_expectations.get("expected_columns", [])
            + table_expectations.get("expected_rows", [])
            + table_expectations.get("expected_elements", [])
        )
        ...
        table_validation: Optional[Dict[str, Any]] = None

        if table_expectations.get("is_table"):
            table_validation = evaluate_table_evidence(
                tag=tag,
                expected_columns=table_expectations.get("expected_columns", []),
                expected_rows=table_expectations.get("expected_rows", []),
                expected_elements=table_expectations.get("expected_elements", []),
                evidence_snippets=ev_snips,
            )
            table_status = (table_validation.get("status") or "").upper()
            if table_status in ("MISSING", "PARTIAL"):
                status = table_status
            if table_validation.get("justification"):
                justification = (
                    f"{justification} {table_validation['justification']}".strip()
                    if justification
                    else table_validation["justification"]
                )
            if not isinstance(result_citations, dict):
                result_citations = {}
            table_citations = table_validation.get("citations") or []
            if table_citations:
                result_citations["table"] = table_citations
        ...
        if table_validation and (table_validation.get("status") or "").upper() != "PRESENT":
            promote_semantic_match = False
