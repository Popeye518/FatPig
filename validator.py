...
def _find_first_available_executable(candidates: List[str]) -> Optional[str]:
...
def run_extractor_preflight(template_path: str) -> None:
    """Validate optional native extractor dependencies before PDF processing starts."""
    if not (template_path or "").strip().lower().endswith(".pdf"):
        return

    issues: List[str] = []

    if importlib.util.find_spec("camelot") is not None:
        ghostscript_path = _find_first_available_executable(["gswin64c", "gswin32c", "gs"])
        if ghostscript_path:
            logging.info("Extractor preflight: Ghostscript found at %s", ghostscript_path)
        else:
            issues.append(
                "camelot-py is installed but Ghostscript is not available on PATH. "
                "Install Ghostscript so table extraction fallback can run."
            )
    else:
        logging.warning(
            "Extractor preflight: camelot-py is not installed; PDF table extraction will rely on pdfplumber only."
        )

    if importlib.util.find_spec("pytesseract") is not None:
        tesseract_path = _find_first_available_executable(["tesseract"])
        if tesseract_path:
            logging.info("Extractor preflight: Tesseract found at %s", tesseract_path)
        else:
            issues.append(
                "pytesseract is installed but tesseract.exe is not available on PATH. "
                "Install Tesseract OCR so scanned-page fallback can run."
            )
    else:
        logging.warning(
            "Extractor preflight: pytesseract is not installed; OCR fallback for scanned PDFs is disabled."
        )

    if issues:
        raise RuntimeError("Extractor preflight failed:\n- " + "\n- ".join(issues))
...
def _clean_extracted_text(text_in: str) -> str:
...
def _new_structured_document(source_path: str, doc_type: str) -> Dict[str, Any]:
    """Create a normalized structured document container."""
    return {
...
def _register_extractor(document: Dict[str, Any], extractor_name: str) -> None:
    """Track which extractors contributed to the structured document."""
    extractors = document.setdefault("extractors", [])
...
def _append_document_section(
    ...
    """Append a normalized section to the structured document."""
    cleaned_content = _clean_extracted_text(content)
...
def _format_table_rows(rows: List[List[Any]]) -> str:
    """Convert extracted table rows into a prompt-friendly string."""
    formatted_rows: List[str] = []
    for row in rows or []:
        cleaned_cells: List[str] = []
        for cell in row or []:
            normalized_cell = _clean_extracted_text(str(cell or "")).replace("\n", " / ")
            if normalized_cell:
                cleaned_cells.append(normalized_cell)
        if cleaned_cells:
...
def _extract_page_ocr_text(page: Any) -> str:
    """Run OCR on a PDF page when native text extraction is empty."""
    try:
        import pytesseract
    ...
    try:
        ...
        return _clean_extracted_text(pytesseract.image_to_string(img) or "")
    except Exception as exc:
        logging.warning("OCR fallback failed for PDF page %s: %s", getattr(page, "number", "?"), exc)
        return ""


def _build_page_spec(page_numbers: List[int]) -> str:
    """Build a Camelot-compatible page selector."""
    normalized_pages = sorted({page for page in page_numbers if page and page > 0})
    return ",".join(str(page) for page in normalized_pages)


def _extract_camelot_tables(pdf_path: str, document: Dict[str, Any]) -> None:
    """Best-effort Camelot table extraction for pages still missing tables."""
    ...
    pages_with_tables = {
        ...
        if section.get("kind") == "table" and section.get("page")
    }
    target_pages = [page for page in range(1, page_count + 1) if page not in pages_with_tables]
    page_spec = _build_page_spec(target_pages)
    if not page_spec:
        return
    ...
    _register_extractor(document, "camelot")

    for flavor in ("lattice", "stream"):
        try:
            tables = camelot.read_pdf(pdf_path, pages=page_spec, flavor=flavor)
        except Exception as exc:
            logging.warning("Camelot %s extraction failed for '%s': %s", flavor, pdf_path, exc)
            continue
        ...
        for table_index, table in enumerate(tables, start=1):
            ...
            rows = []
            data_frame = getattr(table, "df", None)
            ...
            _append_document_section(
                ...
                order=(page_number or 0) * 100 + 60 + table_index,
...
def _render_structured_document(document: Dict[str, Any]) -> str:
    """Render structured sections back into bounded text for embeddings/prompts."""
    sections = document.get("sections", []) or []
    if not sections:
        return ""

    kind_rank = {"text": 0, "table": 1, "diagram": 2}
    ordered_sections = sorted(
        sections,
        key=lambda section: (
            section.get("page") if section.get("page") is not None else 10**9,
            section.get("order", 0),
            kind_rank.get(section.get("kind", "text"), 99),
            section.get("source", ""),
        ),
    )
    ...
    for section in ordered_sections:
        ...
        header = f"[{page_label}{str(section.get('kind', '')).upper()} | {section.get('source', 'unknown')}]"
        rendered_sections.append(f"{header}\n{section.get('content', '')}")
    return "\n\n".join(rendered_sections)


def _log_structured_document_summary(document: Dict[str, Any]) -> None:
    """Log a compact summary of structured document extraction results."""
    sections = document.get("sections", []) or []
    text_count = sum(1 for section in sections if section.get("kind") == "text")
    table_count = sum(1 for section in sections if section.get("kind") == "table")
    diagram_count = sum(1 for section in sections if section.get("kind") == "diagram")
    logging.info(
        "Structured fetch complete for '%s': type=%s pages=%s text=%s tables=%s diagrams=%s extractors=%s",
        ...
        table_count,
        diagram_count,
        ", ".join(document.get("extractors", []) or ["none"]),
    )


def read_pdf_content(pdf_path: str) -> Dict[str, Any]:
    """Extract structured content from a PDF using specialized extractors."""
    document = _new_structured_document(pdf_path, "pdf")
    ...
    try:
        ...
        document["page_count"] = len(fitz_doc)
        _register_extractor(document, "pymupdf")
        ...
        for page_number, page in enumerate(fitz_doc, start=1):
            ...
            try:
                ...
                for block in sorted(raw_blocks, key=lambda item: (item[1], item[0])):
                    if len(block) < 5:
                        continue
                    block_text = _clean_extracted_text(block[4] or "")
                    if block_text:
                        blocks.append(block_text)
            except Exception:
                page_text = _clean_extracted_text(page.get_text("text") or "")
                if page_text:
            ...
            if blocks:
                _append_document_section(
                    ...
                    source="pymupdf",
                    page=page_number,
                    order=page_number * 100,
                )
            else:
                ocr_text = _extract_page_ocr_text(page)
                if ocr_text:
                    _register_extractor(document, "ocr")
                    _append_document_section(
                        document,
                        kind="text",
                        content=ocr_text,
            ...
            for image_index in range(1, image_count + 1):
                _append_document_section(
                    document,
                    kind="diagram",
                    content=f"Embedded diagram/image {image_index} detected on page {page_number}.",
                    source="pymupdf",
                    page=page_number,
                    order=page_number * 100 + 80 + image_index,
                    metadata={"image_index": image_index},
                )
    except Exception as exc:
        logging.warning(f"PyMuPDF failed to extract structured content from '{pdf_path}': {exc}")
    finally:
    ...
    pages_with_text = {
        section.get("page")
        for section in document.get("sections", [])
        if section.get("kind") == "text" and section.get("page")
    }
    try:
        reader = PdfReader(pdf_path)
        document["page_count"] = max(int(document.get("page_count") or 0), len(reader.pages))
        _register_extractor(document, "pypdf2")
        for page_number, page in enumerate(reader.pages, start=1):
            ...
            if page_text:
                _append_document_section(
                    ...
                    source="pypdf2",
                    page=page_number,
                    order=page_number * 100 + 10,
                )
    except Exception as exc:
        logging.warning(f"PyPDF2 failed to extract fallback text from '{pdf_path}': {exc}")

    try:
        import pdfplumber

        with pdfplumber.open(pdf_path) as pdf:
            document["page_count"] = max(int(document.get("page_count") or 0), len(pdf.pages))
            _register_extractor(document, "pdfplumber")
            for page_number, page in enumerate(pdf.pages, start=1):
                try:
                    tables = page.extract_tables() or []
                except Exception as exc:
                    logging.warning("pdfplumber table extraction failed for '%s' page %s: %s", pdf_path, page_number, exc)
                    continue
                for table_index, table in enumerate(tables, start=1):
                    content = _format_table_rows(table)
                    if not content:
                        continue
                    _append_document_section(
                        document,
                        ...
                        source="pdfplumber",
                        page=page_number,
                        order=page_number * 100 + 40 + table_index,
                        metadata={"table_index": table_index},
                    )
    except Exception as exc:
        logging.warning(f"pdfplumber failed to extract structured tables from '{pdf_path}': {exc}")

    _extract_camelot_tables(pdf_path, document)
    return document


def read_pdf_text(pdf_path: str) -> str:
    """Extract structured PDF content and render it as text for downstream use."""
    document = read_pdf_content(pdf_path)
    rendered = _render_structured_document(document)
    if not rendered:
        logging.error(f"Structured PDF fetch returned no content for '{pdf_path}'.")
        return ""
    _log_structured_document_summary(document)
    return rendered


def read_docx_content(docx_path: str) -> Dict[str, Any]:
    """Extract structured content from a DOCX document."""
    document = _new_structured_document(docx_path, "docx")
    doc = DocxDocument(docx_path)
    _register_extractor(document, "python-docx")

    for paragraph_index, paragraph in enumerate(doc.paragraphs, start=1):
        paragraph_text = _clean_extracted_text(paragraph.text or "")
    ...
    for table_index, table in enumerate(doc.tables, start=1):
        ...
        _append_document_section(
            ...
            content=content,
            source="python-docx",
            order=100000 + table_index,
            metadata={"table_index": table_index},
        )

    inline_shapes = getattr(doc, "inline_shapes", None)
    if inline_shapes is not None:
        for shape_index, _ in enumerate(inline_shapes, start=1):
            _append_document_section(
                document,
                kind="diagram",
                content=f"Inline diagram/image {shape_index} detected in document.",
                source="python-docx",
                order=200000 + shape_index,
...
def read_docx_text(docx_path: str) -> str:
    """Extract structured DOCX content and render it as text for downstream use."""
    document = read_docx_content(docx_path)
    rendered = _render_structured_document(document)
    if not rendered:
        logging.error(f"Structured DOCX fetch returned no content for '{docx_path}'.")
        return ""
    _log_structured_document_summary(document)
    return rendered
...
def _structured_section_to_snippet(section: Dict[str, Any]) -> str:
    """Render a structured section into a retrieval-friendly snippet."""
    kind = str(section.get("kind", "text") or "text").strip().upper()
    source = str(section.get("source", "unknown") or "unknown").strip()
    page = section.get("page")
    page_label = f"PAGE {page} " if page is not None else ""
    header = f"[{page_label}{kind} | {source}]"
    return f"{header}\n{section.get('content', '')}".strip()


def _normalize_section_kind(kind: Any) -> str:
    """Normalize a section kind label."""
    return str(kind or "").strip().lower()


def _get_template_document_chunks(
    template_document: Dict[str, Any],
    preferred_kinds: Optional[List[str]] = None,
) -> List[str]:
    """Build indexable chunks directly from structured template sections."""
    sections = template_document.get("sections", []) or []
    if not sections:
        return []

    normalized_preferred_kinds = [
        _normalize_section_kind(kind)
        for kind in (preferred_kinds or [])
        if _normalize_section_kind(kind)
    ]
    if normalized_preferred_kinds:
        preferred_sections = [
            section
            for section in sections
            if _normalize_section_kind(section.get("kind")) in normalized_preferred_kinds
        ]
    ...
    seen = set()
    for section in ordered_sections:
        snippet = _structured_section_to_snippet(section)
        normalized = _normalize_match_text(snippet)
        if not normalized or normalized in seen:
            continue
        seen.add(normalized)
...
def _get_template_pdf_index(
    template_document: Dict[str, Any],
    preferred_kinds: Optional[List[str]] = None,
) -> Tuple[List[str], np.ndarray]:
    """Build and cache a reusable embedding index for the structured template document."""
    chunks = _get_template_document_chunks(template_document, preferred_kinds=preferred_kinds)
    if not chunks:
    ...
    logging.info("Building template document embedding index for %s section chunk(s).", len(chunks))
    cached = (
        chunks,
        np.array(
...
def _build_topic_validation_fallback(
    ...
    if is_diagram:
        ...
        quality = {
            ...
            "table_alignment": 4 if topic_match == "exact" else (2 if evidence_present else 0),
        }

        justification = (
            f"Used deterministic fallback for table requirement '{tag}' because the model returned "
...
def _dedupe_snippets(snippets: List[str], top_n: Optional[int] = None) -> List[str]:
...
def build_requirement_queries(
    requirement_kind: str,
    topic_queries: List[str],
) -> List[str]:
    """Build retrieval queries specialized to the requirement kind."""
    kind_terms = {
        "table": ["table", "matrix", "mapping", "rows", "columns"],
        "diagram": ["diagram", "topology", "architecture", "flow", "component"],
    ...
    queries: List[str] = []
    seen = set()
    for candidate in topic_queries:
        normalized = _normalize_match_text(candidate)
        if normalized and normalized not in seen:
            seen.add(normalized)
            queries.append(candidate)
        enriched = " ".join([candidate] + kind_terms).strip()
        enriched_normalized = _normalize_match_text(enriched)
        if enriched_normalized and enriched_normalized not in seen:
            seen.add(enriched_normalized)
            queries.append(enriched)
    return queries


def build_requirement_search_phrases(
    requirement_kind: str,
    semantic_tag: str,
    tag: str,
    search_phrases: List[str],
) -> List[str]:
    """Build lexical search phrases specialized to the requirement kind."""
    kind_terms = {
        "table": ["table", "matrix", "mapping"],
        "diagram": ["diagram", "topology", "architecture diagram"],
    }.get(requirement_kind, [])

    phrases = list(search_phrases)
    for term in kind_terms:
        phrases.extend([
            f"{semantic_tag} {term}".strip(),
            f"{tag} {term}".strip(),
        ])
    return build_topic_search_phrases(
...
def _snippet_matches_requirement_kind(snippet: str, requirement_kind: str) -> bool:
    """Check whether a retrieved snippet looks relevant for the requested lane."""
    if requirement_kind == "text":
        return True

    normalized = _normalize_match_text(snippet)
    line_tokens = [line for line in re.split(r"\r?\n", snippet or "") if line.strip()]

    if requirement_kind == "table":
        table_markers = ("table", "matrix", "mapping", "inventory", "register")
        if any(marker in normalized for marker in table_markers):
            return True
        pipe_lines = sum(1 for line in line_tokens if "|" in line)
        tab_lines = sum(1 for line in line_tokens if "\t" in line)
        comma_dense_lines = sum(1 for line in line_tokens if line.count(",") >= 2)
        return pipe_lines >= 1 or tab_lines >= 1 or comma_dense_lines >= 2

    if requirement_kind == "diagram":
        diagram_markers = (
            "diagram",
            "topology",
            "architecture",
        ...
        return any(marker in normalized for marker in diagram_markers)

    return True


def retrieve_requirement_evidence_snippets(
    requirement_kind: str,
    topic_queries: List[str],
    semantic_tag: str,
    tag: str,
    ...
    """Retrieve evidence snippets using a lane-aware retrieval strategy."""
    kind_queries = build_requirement_queries(requirement_kind, topic_queries)
    kind_search_phrases = build_requirement_search_phrases(
        requirement_kind=requirement_kind,
        semantic_tag=semantic_tag,
        tag=tag,
        search_phrases=search_phrases,
    )
    merged = retrieve_topic_evidence_snippets(
        kind_queries,
        nar_id,
        release_number,
        rtype,
    ...
    preferred = [
        ...
        if _snippet_matches_requirement_kind(snippet, requirement_kind)
    ]
    if preferred:
        return _dedupe_snippets(preferred + merged, top_n=top_n)
    return merged
...
def should_skip_llm_validation(
    ...
    """Skip LLM validation when no requirement-specific evidence was retrieved."""
    if requirement_kind == "diagram":
        return not evidence_snippets and not (diagram_refs or [])
    return not evidence_snippets
...
def pick_best_template_snippets_for_tag(
    ...
    """Pick best matching structured snippets from the template document using embeddings."""
    chunks, emb_chunks = _get_template_pdf_index(
        template_document,
...
def determine_requirement_kind(
    tag: str,
    notes: str,
    conds: str,
    keywords: List[str],
) -> str:
    """Classify a requirement into text, table, or diagram validation lanes."""
    searchable_text = " ".join(
        [tag, notes, conds] + [keyword for keyword in keywords if isinstance(keyword, str)]
    ).lower()

    if "diagram" in searchable_text or "topology" in searchable_text:
        return "diagram"
    ...
    if any(term in searchable_text for term in table_terms):
        return "table"
...
TABLE_PROMPT = """You are validating whether the required TABLE exists and is of good quality.

Use Template tag/notes, Template-PDF criteria, optional Guidance, and EVIDENCE TEXT SNIPPETS.
Return STRICT JSON only.

Important evaluation rule:
Judge semantic alignment, not exact section numbering.
If the table/content is present under a different numbered section, do NOT penalize that numbering mismatch by itself.
...
{{
    ...
    "justification": "short one or two sentences",
    "citations": {{
        "table": ["short excerpt 1", "short excerpt 2"],
        "guidance": ["short excerpt or empty"],
        "template_pdf": ["short excerpt 1"]
    }}
}}
...
def run_validation(
    ...
    # Extract structured Template PDF/DOCX content
    if template_pdf_path.lower().endswith(".pdf"):
        template_document = read_pdf_content(template_pdf_path)
    elif template_pdf_path.lower().endswith(".docx"):
        template_document = read_docx_content(template_pdf_path)
    else:
        logging.warning("Template PDF path is not PDF/DOCX; skipping PDF guidance extraction.")
        template_document = _new_structured_document(template_pdf_path, "unknown")

    pdf_text = _render_structured_document(template_document)
    if pdf_text:
        _log_structured_document_summary(template_document)
    ...
    for intent_name, child in requirement_nodes:
        ...
        requirement_kind = determine_requirement_kind(
            tag=tag,
            notes=notes,
            conds=conds,
        ...
        ev_snips = retrieve_requirement_evidence_snippets(
            requirement_kind,
            topic_queries,
            semantic_tag,
            tag,
        ...
        pdf_excerpts = pick_best_template_snippets_for_tag(
            ...
            preferred_kinds=(
                ["diagram", "text"]
                if requirement_kind == "diagram"
                else (["table", "text"] if requirement_kind == "table" else ["text"])
            ),
        )
        ...
        is_diagram = requirement_kind == "diagram"
        is_table = requirement_kind == "table"
        ...
        if should_skip_llm_validation(requirement_kind, ev_snips, mm_hits):
            validation_error = "no evidence retrieved; skipped llm call"
            logging.info(
                "Skipping LLM validation for tag '%s' because no %s evidence was retrieved.",
                tag,
                requirement_kind,
            )
            ...
            table_compact_retry_prompt = TABLE_PROMPT.format(
                tag=tag,
                intent=intent_name,
                semantic_tag=semantic_tag,
                notes=_truncate_text(notes, 240),
            ...
            logging.info(
                "Table prompt prepared for tag '%s' with length=%s; compact retry length=%s.",
                tag,
                len(table_prompt),
                len(table_compact_retry_prompt),
            )
            p_json, validation_error = generate_structured_json_with_compact_retry(
                table_prompt,
                table_compact_retry_prompt,
                response_kind=f"table validation for tag '{tag}'",
            )

            if validation_error:
                logging.warning(
                    "Using deterministic table fallback for tag '%s' after model failure.",
                    tag,
                )
                p_json = _build_topic_validation_fallback(
                    tag=tag,
    ...
    arch_pdf = pick_best_template_snippets_for_tag(
        ...
        template_document,
        top_n=3,
        preferred_kinds=["diagram", "text", "table"],
    )
...
def main():
    ...
    run_extractor_preflight(args.template_pdf)
