...
TABLE_EVIDENCE_TEXT_SOURCES = "vs_evidence_text_sources"  # canonical extracted text
...
def _iter_requirement_nodes(
    ...
    for node in nodes:
        ...
        has_requirement_fields = any(
            key in node
            for key in ("musthave", "mustHave", "goodToHave", "goodTohave", "good_to_have", "notes", "conditions", "keywords")
        )
        # Some template leaves only carry a tag and flags; do not silently drop them.
        if tag and (has_requirement_fields or not has_children):
...
def build_topic_search_phrases(
    ...
    def add_phrase(candidate_text: str) -> None:
        raw = (candidate_text or "").strip()
        normalized = _normalize_match_text(raw)
        if raw and normalized and len(normalized) > 3 and normalized not in seen:
            seen.add(normalized)
            phrases.append(raw)
...
def retrieve_keyword_evidence_hits(
    ...
    """Retrieve exact evidence rows using keyword matching."""
    valid_phrases = []
    seen = set()
    ...
    hits = [_build_evidence_hit(row) for row in rows]
    return _dedupe_evidence_hits(hits, limit=top_n)
...
def retrieve_keyword_evidence_snippets(
    ...
    return _evidence_hits_to_snippets(
        retrieve_keyword_evidence_hits(phrases, nar_id, release_number, rtype, doc_id, top_n)
    )


def retrieve_topic_evidence_hits(
    queries: List[str],
    nar_id: str,
    release_number: str,
    rtype: str,
...
def _row_value(row: Any, key: str, default: Any = None) -> Any:
    """Read a column from SQLAlchemy rows, mappings, or simple objects."""
    if row is None:
        return default
...
def _parse_jsonish_dict(value: Any) -> Dict[str, Any]:
    """Convert JSONB-ish values to a plain dict."""
    if isinstance(value, dict):
        return value
...
def _coerce_int(value: Any) -> Optional[int]:
    """Convert scalar values to int when possible."""
    if value in (None, ""):
        return None
    try:
        return int(value)
...
def _build_evidence_hit(row: Any) -> Dict[str, Any]:
    """Normalize a retrieved evidence row into a provenance-rich dict."""
    metadata = _parse_jsonish_dict(_row_value(row, "metadata", {}))
    return {
        "chunk": (_row_value(row, "chunk", "") or "").strip(),
        "chunk_hash": _row_value(row, "chunk_hash"),
        "doc_id": _row_value(row, "doc_id"),
        "doc_uri": _row_value(row, "doc_uri"),
        ...
        "start_offset": _coerce_int(metadata.get("start_offset")),
        "end_offset": _coerce_int(metadata.get("end_offset")),
        "source_text_hash": metadata.get("source_text_hash"),
        "source_id": metadata.get("source_id"),
        "category": metadata.get("category"),
        "text_scope": metadata.get("text_scope"),
...
def _build_source_text_record(row: Any, include_text: bool = True) -> Dict[str, Any]:
    """Normalize a canonical source-text row."""
    metadata = _parse_jsonish_dict(_row_value(row, "metadata", {}))
    record = {
        "doc_id": _row_value(row, "doc_id"),
        "doc_uri": _row_value(row, "doc_uri"),
        "page_num": _coerce_int(_row_value(row, "page_num")),
        "text_scope": _row_value(row, "text_scope"),
        "extractor_name": _row_value(row, "extractor_name"),
        "parser_version": _row_value(row, "parser_version"),
        "canonical_text_hash": _row_value(row, "canonical_text_hash"),
        "source_hash": _row_value(row, "source_hash"),
        "metadata": metadata,
    }
    if include_text:
        record["canonical_text"] = _row_value(row, "canonical_text", "") or ""
    return record


def _evidence_hit_key(hit: Dict[str, Any]) -> str:
    """Build a stable dedupe key for retrieved evidence rows."""
    if hit.get("chunk_hash"):
        return str(hit["chunk_hash"])
    return "|".join(
        str(value or "")
        for value in (
            hit.get("doc_uri"),
...
def _filter_diagram_refs_for_topic(
    ...
    normalized_topic = _normalize_line_text(semantic_tag)
    normalized_search_topic = _normalize_match_text(semantic_tag)
    architecture_family = None
    for family in ("logical", "physical", "conceptual"):
        if family in normalized_search_topic and any(
            marker in normalized_search_topic
            for marker in ("arch", "architect", "diagram", "topology", "deployment")
        ):
    ...
    for ref in refs:
        ...
        search_text = _diagram_ref_search_text(ref)
        normalized_ref_text = _normalize_match_text(search_text)
        family_match = bool(
            architecture_family
            and normalized_ref_text
            and architecture_family in normalized_ref_text
            and any(
                marker in normalized_ref_text
                for marker in ("arch", "architect", "diagram", "topology", "deployment")
            )
        )
        heading_match = bool(
        ...
        if not match_strength and not heading_match and not family_match:
            continue
        enriched = dict(ref)
        enriched["match_strength"] = match_strength
        if family_match and not match_strength:
            enriched["match_strength"] = "family"
        filtered.append(enriched)
    filtered = _dedupe_diagram_refs(filtered, limit=limit)
    if filtered:
        return filtered

    # When image metadata is generic (for example only "Logical Architecture" or
    # "Physical Architecture" headings), avoid dropping all candidates outright.
    if architecture_family:
        family_refs = []
        for ref in refs:
            normalized_ref_text = _normalize_match_text(_diagram_ref_search_text(ref))
            if architecture_family and architecture_family in normalized_ref_text and any(
                marker in normalized_ref_text
                for marker in ("arch", "architect", "diagram", "topology", "deployment")
            ):
                fallback_ref = dict(ref)
                fallback_ref["match_strength"] = "family"
                family_refs.append(fallback_ref)
        family_refs = _dedupe_diagram_refs(family_refs, limit=limit)
        if family_refs:
            return family_refs
...
def _dedupe_evidence_hits(
    ...
    """Keep evidence hits unique by chunk hash or location."""
    deduped: List[Dict[str, Any]] = []
    seen = set()
    for hit in hits:
        key = _evidence_hit_key(hit)
        if not key or key in seen:
            continue
        seen.add(key)
        deduped.append(hit)
...
def _evidence_hits_to_snippets(hits: List[Dict[str, Any]]) -> List[str]:
    """Project evidence hits back to exact stored chunk text for prompting."""
    return [hit["chunk"] for hit in hits if (hit.get("chunk") or "").strip()]


def get_exact_chunks(
    nar_id: str,
    release_number: str,
    ...
    """Return exact stored chunks in deterministic document order."""
    where = ["nar_id=:nar", "release_number=:rel"]
    params: Dict[str, Any] = {"nar": nar_id, "rel": release_number}
    if rtype:
        where.append("rtype=:rtype")
        params["rtype"] = rtype
    if doc_id:
        where.append("doc_id=:doc_id")
        params["doc_id"] = doc_id
    ...
    sql = text(f"""
        ...
        FROM {TABLE_EVIDENCE}
        WHERE {" AND ".join(where)}
        ORDER BY doc_uri, COALESCE(page_num, 0), chunk_index
        {limit_sql}
    """)
    engine = get_engine()
    with engine.connect() as conn:
        rows = conn.execute(sql, params).fetchall()
    return [_build_evidence_hit(row) for row in rows]


def get_canonical_source_text(
    nar_id: str,
    release_number: str,
    doc_id: Optional[str] = None,
    ...
    """Return canonical source text rows persisted during ingestion."""
    where = ["nar_id=:nar", "release_number=:rel"]
    params: Dict[str, Any] = {"nar": nar_id, "rel": release_number}
    if doc_id:
        where.append("doc_id=:doc_id")
        params["doc_id"] = doc_id
    if source_hash:
        where.append("source_hash=:source_hash")
        params["source_hash"] = source_hash
    limit_sql = ""
    if limit is not None and limit > 0:
        limit_sql = "LIMIT :limit"
        params["limit"] = limit
    text_select = ", canonical_text" if include_text else ""
    sql = text(f"""
        SELECT doc_id, doc_uri, page_num, text_scope, extractor_name, parser_version,
               canonical_text_hash, source_hash, metadata{text_select}
        FROM {TABLE_EVIDENCE_TEXT_SOURCES}
        WHERE {" AND ".join(where)}
        ORDER BY doc_uri, COALESCE(page_num, 0), text_scope
        {limit_sql}
    """)
    engine = get_engine()
    with engine.connect() as conn:
        rows = conn.execute(sql, params).fetchall()
    return [_build_source_text_record(row, include_text=include_text) for row in rows]
...
def audit_chunk_integrity(
    ...
    """Verify that stored chunks still match their exact canonical source slices."""
    where = ["c.nar_id=:nar", "c.release_number=:rel"]
    params: Dict[str, Any] = {"nar": nar_id, "rel": release_number}
    if rtype:
        where.append("c.rtype=:rtype")
        params["rtype"] = rtype
    if doc_id:
    ...
    count_sql = text(f"""
        SELECT COUNT(*)
        FROM {TABLE_EVIDENCE} c
        WHERE {where_sql}
    """)
    with engine.connect() as conn:
        total_chunk_count = int(conn.execute(count_sql, params).scalar() or 0)

    sample_sql = text(f"""
        SELECT c.chunk, c.chunk_hash, c.doc_id, c.doc_uri, c.page_num, c.chunk_index, c.metadata,
               s.source_hash AS joined_source_hash, s.canonical_text
        FROM {TABLE_EVIDENCE} c
        LEFT JOIN {TABLE_EVIDENCE_TEXT_SOURCES} s
          ON s.source_hash = c.metadata->>'source_text_hash'
        WHERE {where_sql}
        ORDER BY c.doc_uri, COALESCE(c.page_num, 0), c.chunk_index
        LIMIT :limit
    """)
    sample_params = dict(params)
    sample_params["limit"] = max(1, limit)
    with engine.connect() as conn:
        rows = conn.execute(sample_sql, sample_params).fetchall()
    ...
    for row in rows:
        hit = _build_evidence_hit(row)
        joined_source_hash = _row_value(row, "joined_source_hash")
        canonical_text = _row_value(row, "canonical_text", "") or ""
        if joined_source_hash:
            source_hashes.add(str(joined_source_hash))
        start_offset = hit.get("start_offset")
        end_offset = hit.get("end_offset")
        if not joined_source_hash or not canonical_text:
            missing_source_count += 1
            continue
        if start_offset is None or end_offset is None or end_offset < start_offset:
            missing_offset_count += 1
            continue
        source_slice = canonical_text[start_offset:end_offset]
        if hit.get("chunk") == source_slice:
            exact_match_count += 1
            continue
        mismatch_samples.append({
            ...
            "chunk": _truncate_text(hit.get("chunk", ""), 240),
            "source_slice": _truncate_text(source_slice, 240),
        })

    audited_chunk_count = len(rows)
    comparable_chunk_count = max(0, audited_chunk_count - missing_source_count - missing_offset_count)
    mismatch_count = len(mismatch_samples)
    exact_match_rate = (
        round((exact_match_count / comparable_chunk_count) * 100.0, 1)
        if comparable_chunk_count
        else 0.0
    )
    return {
        "total_chunk_count": total_chunk_count,
        "audited_chunk_count": audited_chunk_count,
        "audit_limit_applied": total_chunk_count > audited_chunk_count,
        "source_text_row_count": len(source_hashes),
        "comparable_chunk_count": comparable_chunk_count,
...
def _format_evidence_hit_display(hit: Dict[str, Any]) -> str:
    """Render a compact, readable provenance line for a retrieved evidence hit."""
    doc_ref = str(hit.get("doc_uri") or hit.get("doc_id") or "(unknown document)")
    location_bits: List[str] = []
    if hit.get("page_num") is not None:
        location_bits.append(f"page {hit['page_num']}")
    if hit.get("chunk_index") is not None:
        location_bits.append(f"chunk {hit['chunk_index']}")
    start_offset = hit.get("start_offset")
    end_offset = hit.get("end_offset")
    if start_offset is not None and end_offset is not None:
        location_bits.append(f"offsets {start_offset}:{end_offset}")

    scope_bits: List[str] = []
    if hit.get("source_id"):
        scope_bits.append(f"source={hit['source_id']}")
    if hit.get("category"):
        scope_bits.append(f"category={hit['category']}")
    if hit.get("text_scope"):
        scope_bits.append(f"scope={hit['text_scope']}")
    if hit.get("extractor_name"):
        scope_bits.append(f"extractor={hit['extractor_name']}")

    rendered = doc_ref
    if location_bits:
        rendered += " | " + " | ".join(location_bits)
    if scope_bits:
        rendered += " | " + ", ".join(scope_bits)
    return rendered


def format_evidence_provenance(
    hits: List[Dict[str, Any]],
    max_items: int = 4,
) -> List[str]:
    """Render unique, readable provenance lines for evidence hits."""
    rendered: List[str] = []
    seen = set()
    for hit in hits:
        line = _format_evidence_hit_display(hit)
...
def _safe_output_stem(value: str) -> str:
    """Convert arbitrary IDs into filesystem-friendly output stems."""
    cleaned = re.sub(r"[^A-Za-z0-9._-]+", "_", (value or "").strip())
    cleaned = cleaned.strip("._")
    return cleaned[:80] or "source_export"


def build_canonical_source_export(
    nar_id: str,
    release_number: str,
    rtype: str,
    doc_id: str,
) -> Dict[str, Any]:
    """Build a canonical source-text export payload for a selected doc_id."""
    source_rows = get_canonical_source_text(
        nar_id=nar_id,
        release_number=release_number,
        doc_id=doc_id,
        include_text=True,
    )
    exact_chunks = get_exact_chunks(
        nar_id=nar_id,
        release_number=release_number,
        rtype=rtype,
        doc_id=doc_id,
    )
    integrity_audit = audit_chunk_integrity(
        nar_id=nar_id,
        release_number=release_number,
        rtype=rtype,
        doc_id=doc_id,
        limit=max(200, len(exact_chunks) or 1),
    )
    return {
        ...
        "rtype": rtype,
        "doc_id": doc_id,
        "source_text_row_count": len(source_rows),
...
def write_canonical_source_export(output_path: str, payload: Dict[str, Any]) -> str:
    """Write canonical source export payload to disk as JSON."""
...
def retrieve_evidence_hits(
    ...
    """Retrieve exact evidence rows using semantic search."""
...
def retrieve_evidence_snippets(
    ...
    return _evidence_hits_to_snippets(
        retrieve_evidence_hits(query, nar_id, release_number, rtype, doc_id, top_n)
    )
...
def run_validation(
    ...
    evidence_integrity_audit = audit_chunk_integrity(
        nar_id=nar_id,
        release_number=release_number,
        rtype=rtype,
        doc_id=effective_doc_id,
        limit=max(50, min(500, top_k * 40)),
    )
    ...
    for intent_name, child in requirement_nodes:
        ...
        ev_hits = retrieve_topic_evidence_hits(
            topic_queries,
            nar_id,
            release_number,
            rtype,
        ...
        ev_snips = _evidence_hits_to_snippets(ev_hits)
        ...
        results.append({
            ...
            "evidence_hits": ev_hits[:top_k],
            "evidence_provenance": format_evidence_provenance(ev_hits[:top_k], max_items=3),
    ...
    arch_hits = retrieve_evidence_hits(
        arch_query, nar_id, release_number, rtype, effective_doc_id, top_n=18
    )
    arch_snips = _evidence_hits_to_snippets(arch_hits)
    arch_mm_hits = (
    ...
    summary = {
        ...
        "architecture_evidence_hits": arch_hits[:8],
        "architecture_evidence_provenance": format_evidence_provenance(arch_hits[:8], max_items=5),
        "architecture_diagram_ref_count": len(arch_mm_hits),
        "architecture_template_excerpt_count": len(arch_pdf),
        "good_to_have_met": good_met,
        "evidence_integrity_audit": evidence_integrity_audit,
...
def generate_summary_pdf(
    ...
    try:
        ...
        integrity_audit = summary.get("evidence_integrity_audit", {}) or {}
        architecture_provenance = summary.get("architecture_evidence_provenance", []) or []
        canonical_source_export = summary.get("canonical_source_export", {}) or {}
        ...
        for item in per_intent:
            if item.get("status") != "PRESENT":
                ...
                requirement_kind = (
                    "must-have" if item.get("musthave") else (
                        "good-to-have" if item.get("good_to_have") else "optional"
                    )
                )
                justification = item.get("justification", "") or "No justification available."
                justification_text = (
                    f"[{requirement_kind} | {status_val}] {justification}"
                    if status_val else justification
                )
                table_data.append([
        ...
        if found_rows == 0:
            table_data.append([
                ...
                Paragraph(
                    "All evaluated requirements are PRESENT, so there is no justification gap.",
                    normal_style,
                ),
            ])
        ...
        story.append(section_heading("Evidence Integrity Audit"))
        story.append(Spacer(1, 8))

        audit_table_data = [
            [
                Paragraph("<b>Total Chunks</b>", small_style),
                Paragraph("<b>Audited</b>", small_style),
                Paragraph("<b>Exact Matches</b>", small_style),
                Paragraph("<b>Match Rate</b>", small_style),
            ],
            [
                Paragraph(str(integrity_audit.get("total_chunk_count", 0)), small_style),
                Paragraph(str(integrity_audit.get("audited_chunk_count", 0)), small_style),
                Paragraph(str(integrity_audit.get("exact_match_count", 0)), small_style),
                Paragraph(f"<b>{integrity_audit.get('exact_match_rate_pct', 0.0):.1f}%</b>", small_style),
            ],
            [
                Paragraph("<b>Mismatches</b>", small_style),
                Paragraph("<b>Missing Source</b>", small_style),
                Paragraph("<b>Missing Offset</b>", small_style),
                Paragraph("<b>Source Rows</b>", small_style),
            ],
            [
                Paragraph(str(integrity_audit.get("mismatch_count", 0)), small_style),
                Paragraph(str(integrity_audit.get("missing_source_count", 0)), small_style),
                Paragraph(str(integrity_audit.get("missing_offset_count", 0)), small_style),
                Paragraph(str(integrity_audit.get("source_text_row_count", 0)), small_style),
            ],
        ]
        audit_table = Table(audit_table_data, colWidths=[120, 120, 120, 120])
        audit_table.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#d9eaf7")),
            ("BACKGROUND", (0, 2), (-1, 2), colors.HexColor("#d9eaf7")),
            ("BACKGROUND", (0, 1), (-1, 1), colors.HexColor("#f9f9f9")),
        ...
        mismatch_samples = integrity_audit.get("mismatch_samples", []) or []
        if mismatch_samples:
            story.append(Paragraph("Mismatch samples", normal_style))
            mismatch_rows = [[
                Paragraph("<b>Location</b>", small_style),
                Paragraph("<b>Stored Chunk</b>", small_style),
                Paragraph("<b>Source Slice</b>", small_style),
            ]]
            for sample in mismatch_samples[:3]:
                location_bits = [str(sample.get("doc_uri") or "")]
                if sample.get("page_num") is not None:
                    location_bits.append(f"page {sample['page_num']}")
                if sample.get("chunk_index") is not None:
                    location_bits.append(f"chunk {sample['chunk_index']}")
                mismatch_rows.append([
                    Paragraph(safe_pdf_text(" | ".join(bit for bit in location_bits if bit)), small_style),
                    Paragraph(safe_pdf_text(sample.get("chunk", "")), small_style),
                    Paragraph(safe_pdf_text(sample.get("source_slice", "")), small_style),
                ])
            mismatch_table = Table(mismatch_rows, colWidths=[120, 185, 185])
        ...
        provenance_rows = [[Paragraph("<b>Requirement</b>", normal_style), Paragraph("<b>Evidence Provenance</b>", normal_style)]]
        provenance_count = 0
        prioritized_items = sorted(
            [item for item in per_intent if item.get("evidence_provenance")],
            key=lambda item: (0 if item.get("musthave") and item.get("status") != "PRESENT" else 1, item.get("tag", "")),
        )
        for item in prioritized_items[:6]:
            provenance_rows.append([
                Paragraph(safe_pdf_text(item.get("tag") or item.get("intent") or "N/A"), normal_style),
                Paragraph(safe_pdf_text("\n".join(item.get("evidence_provenance") or [])), normal_style),
            ])
            provenance_count += 1

        if provenance_count:
            story.append(section_heading("Provenance Highlights"))
            story.append(Spacer(1, 8))
            provenance_table = Table(provenance_rows, colWidths=[160, 330])
        ...
        if architecture_provenance:
            story.append(Spacer(1, 6))
            story.append(Paragraph("Architecture evidence provenance", normal_style))
            story.append(Paragraph(safe_pdf_text("\n".join(architecture_provenance[:5])), small_style))
        if canonical_source_export:
            story.append(Spacer(1, 6))
            export_text = (
                f"Canonical source export: doc_id={canonical_source_export.get('doc_id', '')}; "
                f"source rows={canonical_source_export.get('source_text_row_count', 0)}; "
                f"exact chunks={canonical_source_export.get('exact_chunk_count', 0)}; "
                f"match rate={canonical_source_export.get('exact_match_rate_pct', 0.0):.1f}%"
            )
            story.append(Paragraph(safe_pdf_text(export_text), small_style))
        story.append(Spacer(1, 12))
...
def main():
    ...
    ap.add_argument(
        "--export-source-out",
        default="",
        help="Optional output path for canonical source export JSON.",
    )
    ...
    export_source_requested = bool(
        args.export_source_doc_id.strip() or args.export_source_out.strip()
    )
    ...
    if export_source_requested and export_source_doc_id:
        export_output_path = args.export_source_out.strip() or (
            f"canonical_source_{_safe_output_stem(export_source_doc_id)}.json"
        )
        if not export_output_path.lower().endswith(".json"):
            export_output_path = f"{export_output_path}.json"
        export_payload = build_canonical_source_export(
            nar_id=args.nar_id,
            release_number=args.release_number,
            rtype=rtype,
            doc_id=export_source_doc_id,
        )
        write_canonical_source_export(export_output_path, export_payload)
        result.setdefault("summary", {})["canonical_source_export"] = {
            "doc_id": export_source_doc_id,
            "output_path": export_output_path,
            "source_text_row_count": export_payload.get("source_text_row_count", 0),
            "exact_chunk_count": export_payload.get("exact_chunk_count", 0),
            "exact_match_rate_pct": (
                export_payload.get("integrity_audit", {}) or {}
            ).get("exact_match_rate_pct", 0.0),
            "mismatch_count": (
                export_payload.get("integrity_audit", {}) or {}
            ).get("mismatch_count", 0),
        }
        logging.info(f"Done. Canonical source export: {export_output_path}")
